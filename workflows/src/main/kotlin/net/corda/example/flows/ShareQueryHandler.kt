...
// The oracle flows to handle Share Price queries.
@InitiatedBy(QuerySharePrice::class)
class ShareQueryHandler(val session: FlowSession) : FlowLogic<Unit>() {
    companion object {
        object RECEIVING : ProgressTracker.Step("Receiving query request.")
        object CALCULATING : ProgressTracker.Step("Get Share Price.")
    ...
    override fun call() {
        ...
        val response = try {
            ...
            serviceHub.cordaService(SharePriceOracle::class.java).query(request)
        } catch (e: Exception) {
