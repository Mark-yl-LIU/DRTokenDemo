...
class SharePriceOracle(val services: ServiceHub) : SingletonSerializeAsToken() {


    //PoC Only Support GBP and CNY
    //Product, should use Currency Type and Currency Pair class
    private val ShareList = listOf("GB0005405286", "CNE000001R84" )

    //Default FX Rate for GBP and CNY
    val shareprice = mapOf("GB0005405286" to Amount.parseCurrency("4.93 GBP"),
                            "CNE000001R84" to Amount.parseCurrency("64.73 CNY"))

    private val myKey = services.myInfo.legalIdentities.first().owningKey

    // Returns the Currency Pair Exchange Rate from File / Not found.
    // Change to get the FX rate from Txt Files
    fun query(Symbol : String): Amount<Currency> {
        val ans = shareprice.getOrDefault(Symbol,Amount.parseCurrency("0.01 GBP"))
        if (ans.quantity == 1.toLong())
        {
            throw IllegalArgumentException ("ShareOracle in PoC just support HSBC Share {$Symbol}.")
        }
        else
        {
            return ans
    ...
    fun sign(ftx: FilteredTransaction): TransactionSignature {
        ...
        fun isCommandWithCorrectPrimeAndIAmSigner(elem: Any) = when {
            elem is Command<*> && elem.value is ShareContract.Commands.Create -> {
                val cmdData = elem.value as ShareContract.Commands.Create
                myKey in elem.signers && query(cmdData.Symbol) == cmdData.Price
            }
            else -> false
        }
