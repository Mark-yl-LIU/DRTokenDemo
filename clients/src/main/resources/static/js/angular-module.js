...
app.controller('DemoAppController', function($http, $location, $uibModal) {
    ...
    demoApp.getCurrencyVault = () => $http.get(apiBaseURL + "money")
        .then((response) => demoApp.money = Object.keys(response.data)
            .map((key) => response.data[key].state.data)
            .reverse());

    demoApp.getMyCurrencyVault = () => $http.get(apiBaseURL + "holding")
        .then((response) => demoApp.myholding = Object.keys(response.data)
            .map((key) => response.data[key].state.data)
            .reverse());

    demoApp.getCurrencyVault();
    demoApp.getMyCurrencyVault();
});
